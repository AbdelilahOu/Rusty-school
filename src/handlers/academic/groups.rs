...
//
pub async fn create(body: Json<Group>, state: State) -> HttpResponse {
...
pub async fn list_by_level_id(id: Path<Uuid>, state: State) -> HttpResponse {
    ...
    match res {
        Ok(groups) => HttpResponse::Ok().json(ResponseData {
            error: None,
...
pub async fn list(query: Query<GroupQuery>, state: State) -> HttpResponse {
...
pub async fn update(id: Path<Uuid>, body: Json<Group>, state: State) -> HttpResponse {
    let res =
