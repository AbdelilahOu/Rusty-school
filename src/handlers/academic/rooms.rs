...
pub async fn create(req: Request, body: Json<Room>, state: State) -> Response {
...
pub async fn list(req: Request, query: Query<RoomQuery>, state: State) -> Response {
    let headers = req.headers();
    let authorized = auth_guard(headers, state.config.jwt_secret.clone());
    if let Err(message) = authorized {
        return Response::Unauthorized().json(ResponseData::<String> {
    ...
    match res {
        Ok(rooms) => Response::Ok().json(ResponseData {
            error: None,
...
pub async fn update(req: Request, id: Path<Uuid>, body: Json<Room>, state: State) -> Response {
    let headers = req.headers();
    let authorized = auth_guard(headers, state.config.jwt_secret.clone());
    if let Err(message) = authorized {
        return Response::Unauthorized().json(ResponseData::<String> {
    ...
    let res = MutationService::update_room(&state.db_conn, id.into_inner(), body.into_inner()).await;
    match res {
